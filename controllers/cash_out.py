...
def create():
    ...
    redirect(URL('analytics', 'cash_out', vars=dict(id_seller=seller.id, id_cash_out=new_cash_out_id) ))
...
def update():
    ...
    res = db(db.cash_out.id == request.args(0)).validate_and_update(**update_dict)
    if res.errors:
        raise HTTP(400)
    else:
